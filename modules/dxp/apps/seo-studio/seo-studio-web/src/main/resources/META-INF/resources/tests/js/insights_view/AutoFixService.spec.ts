...
describe('applyTitle', () => {
	...
	it('posts the title and page URL to the SEO Studio Auto Fix backend', async () => {
		...
		await applyTitle({
			htmlTitle: 'New Title',
			pageURL: 'http://slot4.liferay.test:8120/web/customer/products',
		});
		...
		expect(options.body.get('pageURL')).toBe(
			'http://slot4.liferay.test:8120/web/customer/products'
		);
	});
	...
	it('throws when the response is not ok', async () => {
		...
		await expect(
			applyTitle({
				htmlTitle: 't',
				pageURL: 'http://slot4.liferay.test:8120/web/c/p',
			})
		).rejects.toThrow();
	});
});
