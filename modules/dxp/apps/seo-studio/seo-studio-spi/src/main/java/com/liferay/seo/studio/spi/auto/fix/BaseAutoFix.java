...
public abstract class BaseAutoFix implements AutoFix {
	...
	public Response apply(
		...
		if (Objects.equals(friendlyURLPath, pagePath)) {
			...
			// A hostname with a virtual host dedicated to one site serves
			// pages with no leading site friendly URL segment (e.g.
			// /products), so there is no segment to resolve the site from.
			// Search every site at this host for a matching page instead.
			...
			// A /web/ or /group/ prefix carries a leading site friendly URL
			// segment (e.g. /web/customer/products) that isn't part of the
			// page's own friendly URL and must be split off first.
		...
		int responseCode = _patch(
			accessToken,
			getPatchBody(
				languageId, sitePageJSONObject.getJSONObject("pageSettings"),
				value),
			sitePageURL);
	...
	// The customer instance replaces pageSettings wholesale on PATCH rather
	// than merging it, so priority, navigationSettings, and every other SEO
	// field would otherwise be silently reset to their defaults. Merge the
	// changed field into the page's current settings and send the whole
	// object back unchanged apart from that one field.

	protected String buildPatchBody(
		String fieldName, String languageId,
		JSONObject currentPageSettingsJSONObject, String value) {

		JSONObject seoSettingsJSONObject =
			currentPageSettingsJSONObject.getJSONObject("seoSettings");

		if (seoSettingsJSONObject == null) {
			seoSettingsJSONObject = JSONFactoryUtil.createJSONObject();
		}

		JSONObject fieldJSONObject = seoSettingsJSONObject.getJSONObject(
			fieldName);
	...
	private JSONObject _resolveSitePage(
		...
		if (Objects.equals(pageFriendlyURLPath, StringPool.SLASH)) {
			...
			// The virtual host root has no friendly URL of its own.
			// Liferay renders whichever page has the lowest
			// pageSettings.priority there, regardless of that page's own
			// friendly URL and independent of any "/home" naming
			// convention, which a site is free not to follow.
